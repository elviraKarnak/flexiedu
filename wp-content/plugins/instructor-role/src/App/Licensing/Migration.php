...
namespace LearnDash\Instructor_Role\Licensing;
...
class Migration {
	 ...
	 * @var string
	 */
	private const NOTICE_KEY = 'ld_instructor_role_wisdmlabs_license_migration_error';
	 ...
	 * @var string
	 */
	private const UPGRADE_RAN_OPTION_KEY = 'ld_instructor_role_wisdmlabs_license_migration_ran';
	 ...
	 * @var string
	 */
	private const LEGACY_LICENSE_OPTION_KEY = 'edd_instructor_role_license_key';
	...
	public static function run(): void {
		...
		printf(
			...
			'learndash-instructor-role-notice',
			esc_attr( self::NOTICE_KEY ),
			esc_html__( 'Instructor Role: Authentication Error', 'wdm_instructor_role' ),
			wp_kses(
				$notice_text,
				[
					'a' => [
	...
	public static function maybe_generate_token() {
		...
		// Sanity check.
		if ( file_exists( INSTRUCTOR_ROLE_ABSPATH . '/auth-token.php' ) ) {
			$auth_token = include_once INSTRUCTOR_ROLE_ABSPATH . '/auth-token.php';
		...
		$error_message = sprintf(
			...
			__( 'There was an issue migrating your WisdmLabs license key to a LearnDash authentication token. If the issue persists, please do not hesitate to %1$scontact our support team%2$s for further assistance.', 'wdm_instructor_role' ),
		...
		$response = wp_safe_remote_post(
			INSTRUCTOR_ROLE_LICENSING_CHECK_LICENSE_URL,
			[
				'body'    => [
		...
		if (
			! $wp_filesystem->put_contents(
				INSTRUCTOR_ROLE_ABSPATH . '/auth-token.php',
				"<?php return \"{$token}\";",
				FS_CHMOD_FILE
			)
		) {
