(
	function ($, w, undefined) {
		if (w.footable === undefined || w.foobox === null) { // cspell:disable-line
		...
		{
			// update rows index
			$tbody.find( 'tr:not(.' + detailClass + ')' ).each(
				function () {
					var $row = $( this ),
		...
		function Grid()
		{
				...
				$table.off( '.grid' ).on(
					{
						...
						'footable_sorted.grid footable_grid_created.grid footable_grid_removed.grid': function (event) {
							if (options.showIndex && grid.getItem().length > 0) {
								$table.find( 'tbody tr:not(.' + detailClass + ')' ).each(
									function (index) {
				...
				).on(
					{
						'click.grid': function (event) {
							...
							if (options.showCheckbox) {
								$tr.find( 'input:checkbox.check' ).prop(
									'checked',
									function (index, val) {
		   ...
		   */
			grid.newItem = function (item, index, wait) {
				...
				if ($.isArray( item )) {
					...
					grid.footable.raise(
						grid.footable.options.grid.events.created,
						{
							item: item,
							index: index
						}
		   ...
		   */
			grid.setItem = function (item, index) {
				...
				grid.footable.redraw();
				grid.footable.raise(
					grid.footable.options.grid.events.updated,
					{
						item: item,
						index: index
					}
		   ...
		   */
			grid.removeItem = function (index) {
				...
				if ($.isArray( index )) {
					...
					grid.footable.raise(
						grid.footable.options.grid.events.removed,
						{
							item: result,
							index: index
						}
