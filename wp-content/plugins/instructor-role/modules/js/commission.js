...
jQuery( document ).ready(
	function () {
		// jQuery('.ir-instructors-list-container').on( 'click', '.update_commission', function (e) {
		jQuery('.individual-instructor').on( 'click', '.update_commission', function (e) {
				jQuery( this ).parent().find( '.wdm_ajax_loader' ).show();
				e.preventDefault();
				var update_commission = jQuery( this );
				var anchor_tag_name   = jQuery( this ).attr( 'id' );
				var arr               = anchor_tag_name.split( '_' );
				var instructor_id     = arr[1];
				var value             = jQuery( '#commissionInput_' + instructor_id ).val();
