...
class WRLD_Quiz_Reporting_Tools extends WRLD_Common_Functions {
	...
	/**
	 * Gets the details of a specific quiz question including answers and statistics.
	 *
	 * @since 3.0.0
	 *
	 * @return WP_REST_Response Response object with question details.
	 */
		...
		if ( ! empty( $questions_key ) ) {
			...
			foreach ( $questions_key as $key ) {
				foreach ( $questions[ $key ]['questions'] as $value ) {
					...
					$c_msg         = Quiz_Export_Data::instance()->get_correct_message( $question_id, $question->quiz_id );
					// cspell:disable-next-line .
					echo Quiz_Reporting_Frontend::instance()->show_user_answer( $question->question, $value['questionAnswerData'], $value['statistcAnswerData'], $value['answerType'], $question_no, $value['correct'], $c_msg );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		}
	...
	/**
	 * Gets student dashboard results.
	 *
	 * @since 3.0.0
	 *
	...
	public function wrld_get_student_dashboard_results() {
		...
		if ( ! empty( $data ) ) {
			foreach ( $data as &$statistic ) {
				$extradot                   = strlen( substr( strip_tags( learndash_propanel_get_the_title( $statistic['quiz_post_id'] ) ), 0, 100 ) ) == 100 ? ' ...' : ''; // cspell:disable-line .
				$statistic['quiz_title']    = "<a target='_blank' style='display:flex;text-decoration: none;' href='" . add_query_arg(
					...
					''
				) . "'>" . substr( strip_tags( learndash_propanel_get_the_title( $statistic['quiz_post_id'] ) ), 0, 100 ) . $extradot . ' <span style="margin-left:3px;font-size:15px;text-decoration:none;"  class="dashicons dashicons-external"></span></a>'; // cspell:disable-line .
				...
				$statistic['course_title'] = learndash_propanel_get_the_title( $statistic['course_post_id'] );
				...
				// $statistic['score'] = sprintf( __( '%1$d of %2$d', 'learndash-reports-pro' ), $data['points'], $data['gpoints'] );
				...
				$percentage             = $summarized_data['gpoints'] > 0
					? (int) $summarized_data['points'] / (int) $summarized_data['gpoints'] * 100
					: 100;
				...
				$passing_status          = ( ( $percentage >= $passingpercentage ) ? __( 'Pass', 'learndash-reports-pro' ) : __( 'Fail', 'learndash-reports-pro' ) );
	...
	/**
	 * This method is a callback to the API endpoint, 'qre-live-search', which searches the term
	 * entered in students & quizzes & returns the results
	 *
	 * @return WP_REST_Response
	 */
	public function qre_live_search_results() {
		// Get Inputs.
		$search_term = sanitize_text_field( ! empty( $_GET['search_term'] ) ? $_GET['search_term'] : '' );
		...
		if ( strlen( $search_term ) < 3 ) {
			return new WP_Error( 'query-length', __( 'Search term is short enter at least 3 letters', 'learndash-reports-pro' ) );
		}
		if ( isset( $_GET['wpml_lang'] ) ) {
			do_action( 'wpml_switch_language', sanitize_text_field( wp_unslash( $_GET['wpml_lang'] ) ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- 3rd party hook.
		}
	...
	public static function qre_search_quizzes( $query = '', $skip_search = false ) {
		...
		$results = array_map(
			function ( $quiz ) use ( $query ) {
				$item = array(
					'title' => learndash_propanel_get_the_title( $quiz->ID ),
					'type'  => 'quiz',
					'ID'    => $quiz->ID,
					'icon'  => 'quiz_icon',
				);
	 ...
	 * @param bool   $skip_search   Whether to skip searching and fetch all results.[WARNING: Skipping search isn't recommended. For systems that have large number of users, the system may crash due to memory shortage].
	 *
	 * @return array{ title: string, type: string, ID: int, icon: string }[]
	 */
	public static function qre_search_users( $search_string = '', $skip_search = false ) {
		...
		if ( false === $skip_search ) {
			// Query 1: Search by meta fields (first name, last name).
			$meta_args = [
				'meta_query' => [
					'relation' => 'OR',
					[
			...
			if (
				! empty( $excluded_users )
				&& is_array( $excluded_users )
			) {
				$meta_args['exclude'] = $excluded_users;
				$user_args['exclude'] = $excluded_users;
			}

			if (
				! empty( $excluded_ur )
				&& is_array( $excluded_ur )
			) {
				$meta_args['role__not_in'] = $excluded_ur;
				$user_args['role__not_in'] = $excluded_ur;
			}
			...
			$users_by_meta   = (array) $meta_query->get_results();
			$users_by_fields = (array) $user_query->get_results();

			// Create a map of user IDs to remove duplicates.
			$users_found = [];

			// Add users found by meta fields to the map.
			foreach ( $users_by_meta as $user ) {
				$users_found[ $user->ID ] = $user;
			}

			// Add users found by user fields to the map.
			foreach ( $users_by_fields as $user ) {
				$users_found[ $user->ID ] = $user;
			}

			$users_found = array_values( $users_found );
		} else {
			// For skip_search case, use a single query with all fields.
			$args = [];
	 ...
	 *
	 * @return array User's Group affiliated Courses.
	 */
	...
	public static function check_if_user_ids_actually_present( $user_ids ) {
			...
			$user_ids_present = $wpdb->get_col( $wpdb->prepare( 'SELECT ID FROM ' . $wpdb->prefix . 'users WHERE ID IN (%s)', $user_ids ) );
