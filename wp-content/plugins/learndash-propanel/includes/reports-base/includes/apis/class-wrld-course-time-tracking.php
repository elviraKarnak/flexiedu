 ...
 *
 * cspell:ignore datapoint rtype coursewise modulewise quiztime .
 */
...
class WRLD_Course_Time_Tracking extends WRLD_Common_Functions {
	...
	public function save_quiz_completion_time( $quiz_data, $user ) {
		$time = $this->fetch_user_module_time_spent( $quiz_data['quiz'], $quiz_data['course']->ID, $user->ID );
		update_user_meta( $user->ID, 'quiz_time_' . $quiz_data['quiz'], $time );
	}
	...
	public function enqueue_tracking_script() {
		...
		wp_enqueue_script( 'wrld_time_tracking_script', plugins_url( 'assets/js/time-tracking/index.js', WRLD_REPORTS_FILE ), array( 'jquery' ), LDRP_PLUGIN_VERSION, true );
		...
		wp_localize_script(
			...
			array(
				...
				'message'  => get_option( 'wrld_time_tracking_message', __( 'Are you still on this page?', 'learndash-reports-pro' ) ),
				'btnlabel' => get_option( 'wrld_time_tracking_btnlabel', 'Yes' ), // cspell:disable-line .
			)
		);
	}

	/**
	 * This method adds an time spent entry for a user on a particular course.
	 */
		...
		if ( empty( $is_entry_exist ) ) {
			...
			$insert_id = $wpdb->insert(
				...
				array(
					...
					'post_id'          => $post_id,
					'user_id'          => $user_id,
					'group_id'         => $this->wrld_get_user_course_group_id( $course_id, $user_id ) ?? null,
	 ...
	 * @param integer $user_id      User ID.
	 *
	 * @return array    Time spent value for the supplied params.
	 */
	 ...
	 *
	 * @return int Time spent on a quiz by a user in seconds.
	 */
	...
	public function get_course_time_spent() {
		...
		// Get Inputs.
		global $wpdb;
		$request_data     = array_map( 'sanitize_text_field', wp_unslash( $_GET ) );
		...
		if ( isset( $_GET['wpml_lang'] ) ) {
			do_action( 'wpml_switch_language', sanitize_text_field( wp_unslash( $_GET['wpml_lang'] ) ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- 3rd party hook.
		}
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
				$where  = ' ';
				$where1 = ' ';
				if ( 'all' !== strval( $request_data['duration'] ) ) {
			...
			";
				...
				if ( $results ) {
					...
					$results      = array_filter(
						$results,
						function ( $result ) use ( $course_ids ) {
				...
				if ( $course_count <= 0 ) {
					...
					$response          = array(
						...
						'noData'                     => true,
						'message'                    => __( 'No data available for the selected duration and/or filters', 'learndash-reports-pro' ),
						'time_invervals'             => array(), // cspell:disable-line .
						'learner_counts_in_interval' => array(),
						'learners_avg_data'          => array(),
						'average_time_spent'         => 0,
						'learner_below_average'      => 0,
						'learner_above_average'      => 0,
						'total_learners'             => 0,
						'is_group_enabled'           => get_user_meta( get_current_user_id(), 'wrld_ts_goup_enabled', true ), // cspell:disable-line .
						'is_category_enabled'        => get_user_meta( get_current_user_id(), 'wrld_ts_category_enabled', true ),
			...
			";
				...
				if ( 'open' === $course_price_type ) {
					...
					$response          = array(
						'requestData'                => self::get_values_for_request_params( $request_data ),
						'noData'                     => true,
						'message'                    => sprintf(/* translators: %s: custom label for courses */
				...
				$where = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ? ' AND completion_time IS NOT NULL ' : ' AND completion_time IS NULL ';
				...
				$check_course_users_sql = $wpdb->prepare(
					"
			...
			",
				...
				// Execute the query
				$total_users_in_course = $wpdb->get_var( $check_course_users_sql );

				$msg_no_data = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ?
					sprintf(
						// Translators: %1$s: course label, %2$s: course label.
						__( 'There are no learners yet who are in progress or have completed the selected %1$s. Please select a different %2$s.', 'learndash-reports-pro' ),
						LearnDash_Custom_Label::label_to_lower( 'course' ),
						LearnDash_Custom_Label::label_to_lower( 'course' )
					)
					: sprintf(
						// Translators: %1$s: course label, %2$s: Courses label, %3$s: course label.
						__( 'All the learners have completed the selected %1$s. To view time spent data either toggle to "%2$s completed" or select a different %3$s.', 'learndash-reports-pro' ),
						LearnDash_Custom_Label::label_to_lower( 'course' ),
						LearnDash_Custom_Label::get_label( 'courses' ),
						LearnDash_Custom_Label::label_to_lower( 'course' )
					);
				...
				$total_users_sql = "SELECT COUNT(DISTINCT user_id) AS total_users FROM {$wpdb->prefix}ld_course_time_spent WHERE course_id = $course_id";

				if ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) {
					$user_ids_str    = implode( ',', $accessible_users );
					$total_users_sql = $total_users_sql . " AND user_id IN ($user_ids_str) ";
				}
				$query_average = "
						SELECT AVG(total_time_spent) AS average_time_spent
						FROM {$wpdb->prefix}ld_course_time_spent
				...
				$group_enabled    = get_user_meta( $current_user_id, 'wrld_ts_goup_enabled', true ); // cspell:disable-line .
				$category_enabled = get_user_meta( $current_user_id, 'wrld_ts_category_enabled', true );

				// Average completion Course-wise.
				$response = array(
					'requestData'                => self::get_values_for_request_params( $request_data ),
					'time_invervals'             => $users_label_data, // cspell:disable-line .
					...
					'is_group_enabled'           => $group_enabled,
				...
				$response          = array(
					'code'    => 'no-data',
					'message' => sprintf(/* translators: %s: custom label for courses */
						__( 'No %s available', 'learndash-reports-pro' ),
						\LearnDash_Custom_Label::get_label( 'courses' )
					),
					'data'    => array(
						'requestData'          => self::get_values_for_request_params( $request_data ),
						'updated_on'           => date_i18n( 'Y-m-d H:i:s', current_time( 'timestamp' ) ),
						'is_old_data_migrated' => get_option( 'migrated_course_time_access_data', false ),
					),
				);
				$current_timestamp = current_time( 'timestamp' );
	...
	public function get_course_time_details() {
		...
		$where = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ? ' AND ct.completion_time IS NOT NULL ' : ' AND ct.completion_time IS NULL ';

		// logic for time period
		$timeCondition = '';
		if ( 0 === $time_spent_lower_limit && 0 === $time_spent_upper_limit ) {
			$timeCondition = 'total_time_spent = 0 ';
		} elseif ( $time_spent_lower_limit > 0 && 0 === $time_spent_upper_limit ) {
		...
		// SQL query
		$sql = $wpdb->prepare(
			"SELECT ct.total_time_spent , ct.enrollment_date , u.display_name
		...
		$sql2       = "SELECT * FROM {$table_course_time_spent} ct WHERE $timeCondition AND course_id = $course_id $where LIMIT $items_per_page OFFSET  $new_offset";
	...
	public function get_course_time_spent_csv() {
			...
			if ( $course_count <= 0 ) {
				return new WP_REST_Response(
					array(
						'code'    => 'no-data',
						'message' => __( 'No data available for the selected duration and/or filters', 'learndash-reports-pro' ),
						'data'    => array( 'requestData' => self::get_values_for_request_params( $request_data ) ),
					),
					200
				);
		...
		";
			...
			if ( $file ) {
				...
				fputcsv( $file, array() );
				fputcsv( $file, array( __( 'Courses with Progress in below ranges:', 'learndash-reports-pro' ) ) );
				$th2 = array(
					__( 'Course', 'learndash-reports-pro' ),
					__( 'Time spent by learner(in seconds)', 'learndash-reports-pro' ),
					__( 'Avg. Time spent by all learners(in seconds)', 'learndash-reports-pro' ),
				);
				// Inserts Heading into the csv file.
				if ( ! empty( $th2 ) ) {
					fputcsv( $file, $th2 );
			...
			if ( $file ) {
				...
				fputcsv( $file, array() );
				$headerData = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ? array( 'Learners completed in below ranges:' ) : array( __( 'Learners Progress in below ranges:', 'learndash-reports-pro' ) );
	...
	/**
	 * Sets the course time spent filter preferences for the current user.
	 *
	 * @since 3.0.0
	 *
	 * @return WP_REST_Response Response object with filter settings or error message.
	 */
	public function set_course_time_spent_filter() {
		...
		if ( $current_user_id ) {
			...
			update_user_meta( $current_user_id, 'wrld_ts_goup_enabled', $request_data['group'] ?? false ); // cspell:disable-line .
	...
	public function get_lesson_topic_time_spent() {
		...
		if ( ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) && empty( $accessible_users ) ) {
			return new WP_Error(
				'no-users-accessible',
				__( 'No data found for accessible learners', 'learndash-reports-pro' ),
		...
		if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
			...
			$accessible_users   = ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) ? array_intersect( $accessible_users, array( $request_data['learner'] ) ) : array( $request_data['learner'] );
			...
			$accessible_courses = ( ! is_null( $accessible_courses ) && -1 !== intval( $accessible_courses ) ) ? array_intersect( $courses_selected, $accessible_courses ) : $courses_selected;
	...
	public function get_quiz_completion_time() {
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
				...
				return $this->get_modulewise_quiztime( $request_data['course'], $request_data['topic'], 'object', $accessible_users, $request_data );
				...
				return $this->get_modulewise_quiztime( $request_data['course'], $request_data['lesson'], 'object', $accessible_users, $request_data );
			} elseif ( isset( $request_data['course'] ) && ! empty( $request_data['course'] ) ) {
				if ( isset( $accessible_courses ) && -1 !== intval( $accessible_courses ) ) {
					if ( ! in_array( $request_data['course'], $accessible_courses ) ) {
						$response          = new WP_Error(
				...
				return $this->get_modulewise_quiztime( $request_data['course'], $request_data['course'], 'object', $accessible_users, $request_data );
		...
		$response   = maybe_unserialize( $time_spent_row->option_value );
		if ( is_wp_error( $response ) ) {
			// Convert old cached 'no-data' WP_Error to 200 response.
			if ( 'no-data' === $response->get_error_code() ) {
				return new WP_REST_Response(
					array(
						'code'       => 'no-data',
	 ...
	 * @param string              $return         Return type. Default is 'object'.
	 * @param null|array          $student_filter Array of student IDs for which the quiz data is required. Default is null.
	 * @param array{string,mixed} $request_data   Request data. Default is empty array.
	 *
	...
	public function get_modulewise_quiztime( $course, $module, $return = 'object', $student_filter = null, $request_data = array() ) {
		$course_price_type = learndash_get_course_meta_setting( $course, 'course_price_type' );
		if ( 'open' === $course_price_type ) {
			$error_message = sprintf(/* translators: %s: custom label for courses */
				__( 'Reports for open %s are not accessible for the time-being', 'learndash-reports-pro' ),
				\LearnDash_Custom_Label::get_label( 'courses' )
			);
			$error_data = array( 'requestData' => self::get_values_for_request_params( $request_data ) );
			if ( 'array' === $return ) {
				return new WP_Error( 'no-data', $error_message, $error_data );
			}
			return new WP_REST_Response(
				array(
					'code'    => 'no-data',
		...
		if ( empty( $students ) ) {
			$error_message = __( 'No Students Enrolled', 'learndash-reports-pro' );
			$error_data    = array( 'requestData' => self::get_values_for_request_params( $request_data ) );
			if ( 'array' === $return ) {
				return new WP_Error( 'no-data', $error_message, $error_data );
			}
		...
		if ( empty( $quizzes ) ) {
			$error_message = sprintf(/* translators: %s: custom label for quizzes */
		...
		if ( $student_count <= 0 ) {
			...
			$error_message = sprintf( __( 'No Students have made %s attempts', 'learndash-reports-pro' ), \LearnDash_Custom_Label::label_to_lower( 'quiz' ) );
			$error_data    = array( 'requestData' => self::get_values_for_request_params( $request_data ) );
			if ( 'array' === $return ) {
				return new WP_Error( 'no-data', $error_message, $error_data );
			}
	...
	public function ldrp_seconds_to_time( $input_seconds = 0 ) {
		...
		if ( ! empty( $days ) ) {
			...
			$return .= sprintf( _n( '%s day', '%s days', $days, 'learndash-reports-pro' ), number_format_i18n( $days ) );
		}
		...
		if ( ! empty( $hours ) ) {
			...
			$return .= sprintf( _n( '%s hr', '%s hrs', $hours, 'learndash-reports-pro' ), number_format_i18n( $hours ) );
		}
		...
		if ( ! empty( $minutes ) ) {
			...
			$return .= sprintf( _n( '%s min', '%s min', $minutes, 'learndash-reports-pro' ), number_format_i18n( $minutes ) );
		}
		...
		if ( ! empty( $seconds ) ) {
			...
			$return .= sprintf( _n( '%s sec', '%s sec', $seconds, 'learndash-reports-pro' ), number_format_i18n( $seconds ) );
		}
