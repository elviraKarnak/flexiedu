...
class WRLD_Course_Progress_Info extends WRLD_Common_Functions {
	...
	/**
	 * Returns information about inactive users.
	 *
	 * @since 3.0.0
	 *
	 * @return WP_REST_Response|WP_Error Response object with inactive users data or error.
	 */
			...
			if ( empty( $group_users ) ) {
				return new WP_REST_Response(
					[
						'code'    => 'no-data',
						'message' => sprintf(
							// translators: placeholder: custom label for group.
							__( 'No Users enrolled to this %s.', 'learndash-reports-pro' ),
			...
			if ( empty( $group_courses ) ) {
				return new WP_REST_Response(
					[
						'code'    => 'no-data',
						'message' => sprintf(
							// translators: placeholders: %1$s: custom label for courses. %2$s: custom label for group.
							__( 'No %1$s present in this %2$s.', 'learndash-reports-pro' ),
	...
	public function get_daily_enrollments() {
		...
		$results = WRLD_Revenue_API::get_enrollments_between( $duration['start_date'], $duration['end_date'] );

		// Generate all dates in the range and initialize enrollments array with 0 counts.
		...
		foreach ( $period as $date ) {
			...
			$enrollments[ $target_date ] = [
				'count' => 0,
				'date'  => $target_date,
			];
		}

		// Update enrollments array with actual counts from SQL results.
		foreach ( $results as $row ) {
			$enrollment_date = $row['enrollment_date'];
			$count           = (int) $row['count'];
			if ( isset( $enrollments[ $enrollment_date ] ) ) {
				$enrollments[ $enrollment_date ]['count'] = $count;
				$total_enrollment                        += $count;
			}
		}

		// Convert associative array to indexed array to maintain order.
		$enrollments = array_values( $enrollments );

		// Calculate average Enrollment.
		$average_enrollment = $days > 0 ? round( $total_enrollment / $days ) : 0;
		return new WP_REST_Response(
			array(
				'requestData'       => self::get_values_for_request_params( $request_data ),
				'enrollments'       => $enrollments,
				'averageEnrollment' => sprintf(
					// translators: placeholder: Average enrollment count.
					_n( '%d Learner', '%d Learners', $average_enrollment, 'learndash-reports-pro' ),
					$average_enrollment
				),
			),
			200
	...
	public function get_quiz_completion_rate() {
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			do_action( 'wpml_switch_language', isset( $_GET['wpml_lang'] ) ? sanitize_text_field( wp_unslash( $_GET['wpml_lang'] ) ) : '' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- 3rd party hook.
			$user_role_access   = self::get_current_user_role_access();
			...
			if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
				...
				if ( empty( $students ) ) {
					...
					$response          = array(
						'code'    => 'no-data',
						'message' => __( 'No Students enrolled', 'learndash-reports-pro' ),
						'data'    => array(
							'requestData' => self::get_values_for_request_params( $request_data ),
							'updated_on'  => $current_timestamp,
						),
				...
				if ( empty( $quizzes ) ) {
					...
					$response          = array(
						'code'    => 'no-data',
						'message' => sprintf(/* translators: %s: custom label for quizzes */
	...
	public function get_quiz_passing_rate() {
		...
		if ( ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) && 0 === count( $accessible_users ) ) {
			return new WP_REST_Response(
				array(
					'code'    => 'no-data',
					'message' => __( 'No learners found', 'learndash-reports-pro' ),
		...
		if ( $course_count <= 0 ) {
			return new WP_REST_Response(
				array(
					'code'    => 'no-data',
					'message' => sprintf(/* translators: %s: custom label for quiz */
	...
	public function get_average_quiz_attempts() {
		...
		if ( empty( $request_data['course'] ) ) {
			return new WP_REST_Response(
				array(
					'code'    => 'course-selection-required',
					'message' => sprintf(
		...
		if ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) {
			...
			if ( empty( $accessible_users ) ) {
				return new WP_REST_Response(
					array(
						'code'    => 'no-data',
						'message' => sprintf(/* translators: %s: custom label for course */
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( ! empty( $request_data['topic'] ) ) {
				...
				if ( empty( $students ) ) {
					$response          = array(
						'code'    => 'no-data',
						'message' => __( 'No Data to display', 'learndash-reports-pro' ),
						'data'    => array( 'requestData' => self::get_values_for_request_params( $request_data ) ),
					);
					$current_timestamp = current_time( 'timestamp' );
					$wpdb->insert(
			...
			if ( ! empty( $request_data['lesson'] ) ) {
				...
				if ( empty( $students ) ) {
					$response          = array(
						'code'    => 'no-data',
						'message' => sprintf(/* translators: %s: custom label for lesson */
			...
			foreach ( $courses as $course ) {
				...
				if ( $is_pro_version_active ) {
					$array_data = array(
						...
						'avg_total_time_spent' => 0 == $time ? '-' : sprintf( '%02d:%02d:%02d', (int) ( $time / 3600 ), (int) ( $time / 60 ) % 60, (int) $time % 60 ),
