...
if ( ! class_exists( '\WisdmReportsLearndashBlockRegistry\WRLD_Register_Block' ) ) {
	...
	abstract class WRLD_Register_Block {
		 ...
		 * stores the script & style handles in the object properties for later
		 * use.
		 */
		protected function wrld_register_block_assets() {
			...
			$editor_script_deps['dependencies'][] = 'wrld-common-script';
			...
			// Also localize the frontend script data on the editor script for block preview.
			$frontend_script_variable_name = str_replace( '-', '_', $this->get_front_end_script_handle() );
			$frontend_script_data          = $this->get_script_localization_data( $frontend_script_variable_name );
			wp_localize_script( $editor_script_handle, $frontend_script_variable_name, $frontend_script_data );
			...
			$frontend_script_deps['dependencies'][] = 'qre_export_frontend';
			...
			wp_set_script_translations( $editor_script_handle, 'learndash-reports-pro', $language_files_path );
			wp_set_script_translations( $front_end_script_handle, 'learndash-reports-pro', $language_files_path );
		}
		 ...
		 * Gutenberg block assets of the plugin are stored.
		 */
		...
		/**
		 * Generates & returns the url of the stylesheet to be used as a editor stylesheet for the block.
		 *
		 * @return string
		 */
		protected function get_editor_style_sheet_url() {
			...
			if ( is_rtl() ) {
				$style_name = 'editor-' . $this->block_name . '.rtl.css';
			}
			/**
			 * This filter will allow to make changes in the editor script name for the block.
			 */
		...
		/**
		 * Returns the dependencies autogenerated in the file.
		 *
		 * @return string
		 */
		...
		protected function get_front_end_style_sheet_url() {
			...
			if ( is_rtl() ) {
				$style_name = 'index-' . $this->block_name . '.rtl.css';
			}
			/**
			 * This filter will allow to make changes in the editor script name for the block.
			 */
		...
		/**
		 * Concatenate the block namespace with the block name & returns
		...
		/**
		 * Gets localization data for the script getting registered & localized that data in the script variable.
		 *
