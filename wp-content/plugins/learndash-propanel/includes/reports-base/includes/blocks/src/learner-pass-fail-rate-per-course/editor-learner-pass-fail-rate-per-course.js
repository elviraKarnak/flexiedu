...
import './editor.scss';
import LearnerPassFailRate from './index-learner-pass-fail-rate-per-course.js';
...
registerBlockType(
	'wisdm-learndash-reports/learner-pass-fail-rate-per-course',
	{
		title: __( 'Learners Pass/Fail Rate', 'learndash-reports-pro' ),
		description: __(
		...
		className:
			'learndash-reports-by-wisdmlabs-learner-pass-fail-rate-per-course',
		icon,
		apiVersion: '3',
		attributes: {
			categories: {
		...
		edit( props ) {
			return (
				<div { ...useBlockProps() }>
					<LearnerPassFailRate></LearnerPassFailRate>
				</div>
			);
		},
		...
		save() {
			return (
				<div { ...useBlockProps.save() }>
					<div className="wisdm-learndash-reports-learner-pass-fail-rate-per-course front"></div>
				</div>
			);
		},
	}
