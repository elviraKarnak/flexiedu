...
import './editor.scss';
import CourseProgressRate from './index-course-progress-rate.js';
...
registerBlockType( 'wisdm-learndash-reports/course-progress-rate', {
	title: sprintf(
		// Translators: %s: Course label.
		__( '%s Progress Rate', 'learndash-reports-pro' ),
		wisdm_reports_get_ld_custom_lebel_if_avaiable( 'Course' )
	),
	description: sprintf(
		// Translators: %s: course label.
		__( 'Graph of the %s progress rate', 'learndash-reports-pro' ),
		wisdm_reports_get_ld_custom_lebel_if_avaiable( 'Course', 'lower' )
	),
	category: 'wisdm-learndash-reports',
	className: 'learndash-reports-by-wisdmlabs-course-progress-rate',
	icon,
	apiVersion: '3',
	attributes: {
		categories: {
	...
	save() {
		return (
			<div { ...useBlockProps.save() }>
				<div className="wisdm-learndash-reports-course-progress-rate front"></div>
			</div>
		);
	},
} );
