...
class StudentProfile extends Component {
	constructor( props ) {
		super( props );
		let error = null;
		const avatar = this.getUserType()
			? 'www.gravatar.com/avatar/789047b2eb7fd33f3fb6858358dcc5d8?s=150&r=g&d=mm'
			: wisdm_ld_reports_common_script_data.avatar_url;

		const username = this.getUserType()
			? 'No Name'
			: wisdm_ld_reports_common_script_data.current_user.data
					.display_name;
		if ( this.getUserType() ) {
			error = {
				message: __(
...
document.addEventListener( 'DOMContentLoaded', function ( event ) {
	...
	if ( elem.length > 0 ) {
		...
		root.render( React.createElement( StudentProfile ) );
	}
} );
