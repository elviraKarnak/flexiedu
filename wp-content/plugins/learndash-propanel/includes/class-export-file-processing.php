 ...
 * @since 3.0.0
 *
 * cspell:ignore qstn qsanswer qkey cust qval qsanswer_data .
 */
...
if ( ! class_exists( 'Export_File_Processing' ) ) {
	...
	class Export_File_Processing {
		...
		public function export_course_data_request() {
			...
			$table_headers = $post_data['tableHeaders'];
			$table_data    = json_decode( html_entity_decode( stripslashes( $post_data['tableData'] ) ) );
			...
			foreach ( $table_headers as $header_cell ) {
				$excel_data[0][] = $header_cell['Header'];
			}
			...
			$active_sheet->fromArray( $excel_data, null, 'A1', true );
			$columns_to_auto_resize = array( 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H' );
			array_map(
				function ( $column ) use ( $active_sheet ) {
					$active_sheet->getColumnDimension( $column )->setAutoSize( true );},
				$columns_to_auto_resize
			);
		...
		public function csv_table( $arr_data, $arr_custom_data ) {
			...
			foreach ( $arr_data as $key => $val ) {
				...
				if ( isset( $val['question_meta'] ) ) {
					...
					$percentage = $val['total_points'] > 0
						? round( ( ( $val['tot_points_scored'] / $val['total_points'] ) * 100 ), 2 )
						: 100;
					...
					$table .= '<tr>
	                        ...
	                        <td>' . $val['tot_points_scored'] . ' ( ' . $percentage . '% )</td>
		...
		public function xls_table( $arr_data, $arr_custom_data ) {
			...
			foreach ( $arr_data as $val ) {
				...
				if ( isset( $val['question_meta'] ) ) {
					...
					foreach ( $val['question_meta'] as $qkey => $qval ) {
						$qkey                        = $qkey;
						$table[ $index ][0]['value'] = wp_strip_all_tags( str_replace( '&#39;', "'", html_entity_decode( $qval['question'] ) ) ); // Question Column.
					...
					$table[ $index ][5]['value'] = $val['tot_points_scored'] . ' ( ' . $percentage . '% )';
					$table[ $index ][5]['font']  = array( 'bold' => 1 );
