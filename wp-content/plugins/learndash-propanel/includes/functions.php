 ...
 *
 * cSpell:ignore indexkey columnkey prevresult .
 */
...
if ( ! function_exists( 'check_if_user_is_course_admin' ) ) {
	function check_if_user_is_course_admin( $course_id, $user_id = 0 ) {
		...
		if ( in_array( 'group_leader', (array) $user->roles, true ) ) {
			...
			if ( ! in_array( $course_id, $course_ids ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
				return false;
			}
			return true;
		}
	...
	function qre_custom_reports_pagination_data( $courses, $selected_users_for_groups = null ) {
		...
		foreach ( $courses as $key => $course ) {
			...
			if ( check_if_user_is_course_admin( $course['post']->ID ) ) {
				$stat_args = array( 'quiz_ids' => $quiz_pro_ids );
				if ( null !== $selected_users_for_groups ) {
					$stat_args['user_ids'] = $selected_users_for_groups;
				}
			} else {
				$stat_args = array(
			...
			$statistics_count = Quiz_Export_Db::instance()->get_all_statistic_ref_ids_count( $stat_args );

			if ( empty( $statistics_count ) ) {
				$statistics_count = 1;
			}
			$extra_rows = 0;
