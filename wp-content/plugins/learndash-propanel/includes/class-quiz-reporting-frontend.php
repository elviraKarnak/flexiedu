 ...
 * @since 3.0.0
 *
 * cspell:ignore counterj maxtrix mextrix pqid quet qtime qpoints qscore qcount qids statistc cmsg qkey qstn qval qsanswer qspoints crossreferenced qsanswer_data .
 */
...
use LearnDash\Reports\PhpOffice\PhpSpreadsheet\Writer\Xlsx;
...
if ( ! class_exists( 'Quiz_Reporting_Frontend' ) ) {
	...
	class Quiz_Reporting_Frontend {
		...
		/**
		 * Get all statistic reference ids.
		 *
		...
		public function get_all_statistic_ref_ids( $statistics ) {
			...
			if ( ! empty( $excluded_courses ) ) {
				foreach ( $statistics as $key => $statistic ) {
					if ( in_array( $statistic['course_post_id'], $excluded_courses ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			...
			if ( ! empty( $excluded_users ) ) {
				foreach ( $statistics as $key => $statistic ) {
					if ( in_array( $statistic['user_id'], $excluded_users ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
						unset( $statistics[ $key ] );
					}
				}
			}
		...
		/**
		 * Get all statistic reference id arguments
		 *
			...
			if ( ! empty( $excluded_courses ) ) {
				if ( empty( $args['quiz_ids'] ) ) {
					...
					foreach ( $args['quiz_ids'] as $key => $quiz_id ) {
						...
						if ( in_array( $course_id, $excluded_courses ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			...
			if ( ! empty( $excluded_users ) ) {
				if ( empty( $args['user_ids'] ) ) {
					...
					foreach ( $args['user_ids'] as $key => $user_id ) {
						if ( in_array( $user_id, $excluded_users ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
		...
		/**
		 * Get cross referenced statistics arguments
		 *
		...
		public function get_crossreferenced_statistics_args( $courses ) {
			...
			if ( ! empty( $excluded_courses ) ) {
				foreach ( $courses as $key => $course ) {
					if ( in_array( $course['post']->ID, $excluded_courses ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
						unset( $courses[ $key ] );
					}
				}
			}
		...
		/**
		 * Get all statistics reference ids by user.
		 *
		...
		/**
		 * Get statistic reference ids by user arguments
		 *
		 * @param array   $args     Array of arguments.
		...
		/**
		 * Get all statistic reference ids by quiz.
		 *
		 * @param array $statistics Array of statistics data.
		 * @return array
		 */
		...
		/**
		 * Get statistic reference ids by quiz arguments
		 *
		 * @param array   $args     Array of arguments.
		...
		/**
		 * Migrate incorrect user metadata.
		 */
		...
		public function get_quiz_reports_data() {
			...
			if ( ! wp_verify_nonce( $nonce, 'wisdm_ld_reports_page' ) ) {
		...
		/**
		 * Render default quiz reports.
		 *
		 * @param integer $page_number  Page number. Defaults to 1.
		 * @return string
		 */
		public function render_default_quiz_reports( $page_number = 1 ) {
		...
		public function show_paginated_entries() {
			$query_type     = isset( $_GET['search_result_type'] ) ? sanitize_text_field( wp_unslash( $_GET['search_result_type'] ) ) : '';
			$queried_obj_id = filter_input( INPUT_GET, 'search_result_id', FILTER_VALIDATE_INT );
			$queried_string = isset( $_GET['qre-search-field'] ) ? sanitize_text_field( wp_unslash( $_GET['qre-search-field'] ) ) : '';
			$date_filter    = isset( $_GET['filter_type'] ) ? sanitize_text_field( wp_unslash( $_GET['filter_type'] ) ) : '';
			$time_period    = isset( $_GET['period'] ) ? sanitize_text_field( wp_unslash( $_GET['period'] ) ) : '';
			...
			$page           = isset( $_GET['pageno'] ) ? sanitize_text_field( wp_unslash( $_GET['pageno'] ) ) : '';
		...
		public function wrld_export_entries() {
			...
			$quiz_id    = $quiz_pro_id = $fields['quiz_filter']; // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found
		...
		public function wrld_export_progress_results() {
			...
			$percentage = $total > 0
				? floatval( number_format( 100 * $current / $total, 2, '.', '' ) )
				: 100;
		...
		public function wrld_export_attempt_csv( $data ) {
			...
			if ( $file ) {
				...
				foreach ( $data as $entry ) {
					...
					$pass              = 0;
					$percentage        = $entry['summary']['gpoints'] > 0
						? $entry['summary']['points'] / $entry['summary']['gpoints'] * 100
						: 100;
				...
				foreach ( $data as $quiz_id => $result ) {
					foreach ( $result as $course_id => $statistics_data ) {
						...
						if ( ! empty( $post_terms ) && ! is_wp_error( $post_terms ) ) {
							...
							$quiz_terms = trim( strip_tags( rtrim( trim( str_replace( '<br />', $separator, $quiz_terms ) ), $separator ) ) );
						}
						$headings = array(
						...
						foreach ( $statistics_data as $entry ) {
							...
							$td[] = $entry['summary']['points'];
							$td[] = number_format( (float) $percentage, 2, '.', '' );
		...
		/**
		 * Parse Learner data in XLS format
		 *
			...
			foreach ( $data as $quiz_id => $result ) {
				foreach ( $result as $course_id => $statistics_data ) {
					...
					foreach ( $statistics_data as $entry ) {
						...
						$table[ $index ][7]['value'] = number_format( (float) $percentage, 2, '.', '' );
						$inner_index                 = 7;
						foreach ( $entry['detail'] as $question ) {
		...
		public function parse_xls_data( $data ) {
			...
			foreach ( $data as $entry ) {
				...
				$table[ $index ][11]['value'] = number_format( (float) $percentage, 2, '.', '' );
			...
			$statistic_data = array_filter(
				array_map(
					function ( $statistic ) use ( $data_fields, $course_id, $quiz_id, $filter_fields ) {
						...
						/*
						 * Check if the user who took the quiz is enrolled in the quiz's course.
						 * Skip this check for standalone quizzes (course_id = 0) since they have
						 * no course to be enrolled in - the quiz attempt is still valid.
						 */

						// Cast to int to prevent type mismatch.
						$course_id                = Cast::to_int( $course_id );
						$user_enrolled_course_ids = array_map( 'Cast::to_int', $user_enrolled_course_ids );

						if (
							! empty( $course_id )
							&& ! in_array( $course_id, $user_enrolled_course_ids, true )
						) {
							return null;
						}
						...
						if ( array_key_exists( 'quiz_status', $data_fields ) ) {
							...
							$percentage        = $quiz_data[0]['total_points'] > 0
								? (float) number_format( ( $quiz_data[0]['tot_points_scored'] * 100 ) / $quiz_data[0]['total_points'] )
								: 100;
						...
						if ( array_key_exists( 'quiz_score_percent', $data_fields ) ) {
							...
							$percentage                 = isset( $data['quiz_points_earned'] )
														&& isset( $data['quiz_points_total'] )
														&& $data['quiz_points_total'] > 0
								? $data['quiz_points_earned'] / $data['quiz_points_total'] * 100
								: 100;
							$data['quiz_score_percent'] = number_format( $percentage, 2 );
						}
		...
		/**
		 * This function is used to show the answer to the user in a HTML format
		 *
		 ...
		 * @param string  $cmg           Correct Message.
		 */
			...
			for ( $cnt = 0; $cnt < $max_count; $cnt++ ) {
				...
				if ( 'single' === $answer_type || 'multiple' === $answer_type ) {
					...
					if ( isset( $s_answer_data[ $cnt ] ) && isset( $q_answer_data[ $s_answer_data[ $cnt ] ] ) ) {
						if ( $s_answer_data[ $cnt ] == $cnt ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
					...
					$sort_text = '';
					if ( isset( $s_answer_data[ $cnt ] ) && isset( $q_answer_data[ $s_answer_data[ $cnt ] ] ) ) {
						if ( in_array( $s_answer_data[ $cnt ], $matrix[ $cnt ] ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
							$correct = 'wpProQuiz_answerCorrect';
						}
						$ans_val   = $q_answer_data[ $s_answer_data[ $cnt ] ];
