 ...
 */
class LearnDash_Reports_HTMLPurifier_Strategy_MakeWellFormed extends LearnDash_Reports_HTMLPurifier_Strategy
{
     ...
     * @type LearnDash_Reports_HTMLPurifier_Zipper
     */
    protected $zipper;
     ...
     * @type LearnDash_Reports_HTMLPurifier_Injector[]
     */
    protected $injectors;

    /**
     * Current instance of LearnDash_Reports_HTMLPurifier_Config.
     * @type LearnDash_Reports_HTMLPurifier_Config
     */
    protected $config;

    /**
     * Current instance of LearnDash_Reports_HTMLPurifier_Context.
    ...
    public function execute($tokens, $config, $context)
    {
        ...
        // local variables
        $generator = new LearnDash_Reports_HTMLPurifier_Generator($config, $context);
        $escape_invalid_tags = $config->get('Core.EscapeInvalidTags');
        // used for autoclose early abortion
        $global_parent_allowed_elements = $definition->info_parent_def->child->getAllowedElements($config);
        $e = $context->get('ErrorCollector', true);
        $i = false; // injector index
        list($zipper, $token) = LearnDash_Reports_HTMLPurifier_Zipper::fromArray($tokens);
        if ($token === NULL) {
            return array();
        }
        ...
        for (;;
            ...
            if (is_int($i)) {
                ...
                if (is_int($rewind_offset)) {
                    for ($j = 0; $j < $rewind_offset; $j++) {
                        ...
                        } elseif ($token instanceof LearnDash_Reports_HTMLPurifier_Token_End) {
            ...
            if ($token === NULL) {
                ...
                $token = new LearnDash_Reports_HTMLPurifier_Token_End($top_nesting->name);
            ...
            if (empty($token->is_tag)) {
                if ($token instanceof LearnDash_Reports_HTMLPurifier_Token_Text) {
            ...
            $ok = false;
            if ($type === 'empty' && $token instanceof LearnDash_Reports_HTMLPurifier_Token_Start) {
                // claims to be a start tag but is empty
                $token = new LearnDash_Reports_HTMLPurifier_Token_Empty(
                    $token->name,
                    $token->attr,
                    $token->line,
                    $token->col,
                ...
                $ok = true;
            } elseif ($type && $type !== 'empty' && $token instanceof LearnDash_Reports_HTMLPurifier_Token_Empty) {
                // claims to be empty but really is a start tag
                // NB: this assignment is required
                $old_token = $token;
                $token = new LearnDash_Reports_HTMLPurifier_Token_End($token->name);
                $token = $this->insertBefore(
                    new LearnDash_Reports_HTMLPurifier_Token_Start($old_token->name, $old_token->attr, $old_token->line, $old_token->col, $old_token->armor)
                );
                // punt (since we had to modify the input stream in a non-trivial way)
                $reprocess = true;
                continue;
            } elseif ($token instanceof LearnDash_Reports_HTMLPurifier_Token_Empty) {
                // real empty token
                $ok = true;
            } elseif ($token instanceof LearnDash_Reports_HTMLPurifier_Token_Start) {
                // start tag
                ...
                if (!empty($this->stack)) {
                    ...
                    if ($autoclose && $definition->info[$token->name]->wrap) {
                        ...
                        if (isset($elements[$token->name]) && isset($parent_elements[$wrapname])) {
                            $newtoken = new LearnDash_Reports_HTMLPurifier_Token_Start($wrapname);
                    ...
                    if ($autoclose) {
                        ...
                        if ($autoclose_ok) {
                            ...
                            $new_token = new LearnDash_Reports_HTMLPurifier_Token_End($parent->name);
            ...
            if (!$token instanceof LearnDash_Reports_HTMLPurifier_Token_End) {
                throw new LearnDash_Reports_HTMLPurifier_Exception('Unaccounted for tag token in input stream, bug in HTML Purifier');
            }
            ...
            if (empty($this->stack)) {
                if ($escape_invalid_tags) {
                    ...
                    $token = new LearnDash_Reports_HTMLPurifier_Token_Text($generator->generateFromToken($token));
                } else {
                    if ($e) {
            ...
            for ($j = 1; $j < $c; $j++) {
                ...
                $new_token = new LearnDash_Reports_HTMLPurifier_Token_End($skipped_tags[$j]->name);
     ...
     *
     * @param LearnDash_Reports_HTMLPurifier_Token|array|int|bool $token Token substitution value
     * @param LearnDash_Reports_HTMLPurifier_Injector|int $injector Injector that performed the substitution; default is if
    ...
    protected function processToken($token, $injector = -1)
    {
        ...
        if (!is_array($token)) {
            throw new LearnDash_Reports_HTMLPurifier_Exception('Invalid token type from injector');
        }
        if (!is_int($token[0])) {
            array_unshift($token, 1);
        }
        if ($token[0] === 0) {
            throw new LearnDash_Reports_HTMLPurifier_Exception('Deleting zero tokens is not valid');
        }
...
// property of LearnDash_Reports_HTMLPurifier_Token was to help avoid infinite loops
...
//
//  - With LearnDash_Reports_HTMLPurifier_Injector_RemoveEmpty, we noticed that if
