 ...
 */
class LearnDash_Reports_HTMLPurifier_Injector_Linkify extends LearnDash_Reports_HTMLPurifier_Injector
{
    /**
     * @type string
     */
    ...
    public function handleText(&$token)
    {
        ...
        for ($i = 0, $c = count($bits), $l = false; $i < $c; $i++, $l = !$l) {
            if (!$l) {
                ...
                $token[] = new LearnDash_Reports_HTMLPurifier_Token_Start('a', array('href' => $bits[$i]));
                $token[] = new LearnDash_Reports_HTMLPurifier_Token_Text($bits[$i]);
