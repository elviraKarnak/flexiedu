 ...
 * pales in comparison to LearnDash_Reports_HTMLPurifier_Lexer_DOMLex.
 *
 * @todo Reread XML spec and document differences.
 */
class LearnDash_Reports_HTMLPurifier_Lexer_DirectLex extends LearnDash_Reports_HTMLPurifier_Lexer
{
    /**
     * @type bool
     */
    ...
    public function tokenizeHTML($html, $config, $context)
    {
        ...
        while (++$loops) {
            ...
            if (!$inside_tag && $position_next_lt !== false) {
                ...
                $token = new
                LearnDash_Reports_HTMLPurifier_Token_Text(
                    $this->parseText(
                        substr(
                            $html,
                            $cursor,
                ...
                if (substr($segment, 0, 3) === '!--') {
                    ...
                    $segment = substr($html, $cursor, $strlen_segment);
                    $token = new
                    LearnDash_Reports_HTMLPurifier_Token_Comment(
                        substr(
                            $segment,
                            3,
                            $strlen_segment - 3
                ...
                if ($is_end_tag) {
                    ...
                    $token = new LearnDash_Reports_HTMLPurifier_Token_End($type);
                    if ($maintain_line_numbers) {
                        $token->rawPosition($current_line, $current_col);
                ...
                if ($position_first_space >= $strlen_segment) {
                    if ($is_self_closing) {
                        $token = new LearnDash_Reports_HTMLPurifier_Token_Empty($segment);
                    } else {
                        $token = new LearnDash_Reports_HTMLPurifier_Token_Start($segment);
                    }
                    if ($maintain_line_numbers) {
                        $token->rawPosition($current_line, $current_col);
                        $current_line += substr_count($html, $nl, $cursor, $position_next_gt - $cursor);
                ...
                if ($is_self_closing) {
                    $token = new LearnDash_Reports_HTMLPurifier_Token_Empty($type, $attr);
                } else {
                    $token = new LearnDash_Reports_HTMLPurifier_Token_Start($type, $attr);
                }
                if ($maintain_line_numbers) {
                    $token->rawPosition($current_line, $current_col);
                    $current_line += substr_count($html, $nl, $cursor, $position_next_gt - $cursor);
