...
class Rels extends WriterPart
{
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, $worksheetId = 1, $includeCharts = false, $tableRef = 1)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    private function writeUnparsedRelationship(\LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, XMLWriter $objWriter, string $relationship, string $type): void
    {
        $unparsedLoadedData = $worksheet->getParentOrThrow()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            ...
            if (
                $drawing instanceof \LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Drawing
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    public function writeVMLDrawingRelationships(\LearnDash\Reports\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet): string
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
