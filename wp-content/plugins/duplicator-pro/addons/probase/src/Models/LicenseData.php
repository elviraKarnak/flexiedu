...
class LicenseData extends AbstractEntity
{
    ...
    public function __wakeup()
    {
        try {
            /**
             * When the object is unserialized, check whether the license data is stored as an encrypted string.
             *
             * - If $this->data is a string, it represents encrypted license data that must be decrypted.
             * - If it is not a string, the data is assumed to already be in its usable (decrypted) form.
             *
             * Note: The license data may remain unencrypted in the database even when encryption is enabled by default.
             *       This can occur until the settings are saved and the encryption process is applied.
             */
            if (is_string($this->data)) {
                // Data is encrypted, decrypt all fields
                $this->licenseKey = (string) CryptBlowfish::decryptIfAvaiable((string) $this->licenseKey, null, true);
                ...
                $decryptedData = CryptBlowfish::decryptIfAvaiable($this->data, null, true);
                if (strlen($decryptedData)) {
                    $this->data = JsonSerialize::unserialize($decryptedData);
                } else {
                    throw new Exception('Error on license data decryption');
                }
            }
            ...
            if (!self::isValidLicenseFormat($this->licenseKey)) {
    ...
    public function deactivate(): int
    {
        ...
        DupLog::trace("DEACTIVATE LICENSE: " . self::maskLicenseKey($this->licenseKey));
